# Diagramas de secuencia — M01 Autenticación y roles
...
sequenceDiagram
    ...
    alt intentos_fallidos menor que 5
        SRV-->>API: Error de credenciales
        API-->>NG: 401 Usuario o contrasena incorrectos
    else intentos_fallidos igual a 5
        SRV->>DB: Marcar bloqueo hasta ahora mas 15 min
...
## S-M01-03 · Acceso a endpoint protegido (HU-M01-03, CA02)

```mermaid
sequenceDiagram
    participant NG as Angular
    participant INT as Interceptor de token
    participant API as Django REST
    ...
    alt Token invalido o expirado
        ...
        alt Rol sin permiso
            PER-->>API: Denegado
            API->>AUD: Registrar evento ACCESO_RECHAZADO
