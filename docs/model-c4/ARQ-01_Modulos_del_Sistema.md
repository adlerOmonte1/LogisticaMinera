...
**Documento:** ARQ-01
**Versión:** 1.1
**Estado:** Aprobado — base del indicador I1 de la variable independiente

---

## 1. Propósito

Fijar la lista cerrada de módulos que constituyen el sistema web. Esta lista es el **denominador** del indicador de la variable independiente "porcentaje de módulos implementados sobre módulos planificados". Toda ampliación posterior baja ese porcentaje; toda reducción debe documentarse antes de firmar la ficha técnica.
...
- **Un indicador de la variable dependiente** que no puede mejorar sin ese módulo.
- **Una restricción documentada** en el Capítulo I (conectividad intermitente en planta, ausencia de registro de volquetes propios, pérdida del reporte mensual).
...
**M03 — Registro de ingresos.** Es el núcleo. La unidad de análisis de toda la tesis es el ingreso de volquete a planta. I1 (latencia entre pesaje y disponibilidad del dato) e I2 (cobertura) se miden directamente sobre los registros que produce este módulo.
...
**M08 — Auditoría.** No mejora ningún indicador de la variable dependiente, pero sostiene la validez de los datos recolectados: permite demostrar ante el jurado que los registros del postest no fueron alterados retroactivamente.
...
Se declara explícitamente para proteger el cronograma de desarrollo y para poder responder en sustentación:
...
M07 se aborda en la semana 4 y no al final, por la razón indicada en `decisiones_diseno.md` §4. La semana 8 es de estabilización y **no** de desarrollo: es donde se llenan los instrumentos de la variable independiente.
...
| Volumen de datos insuficiente en la ventana de observación | El contraste inferencial pierde potencia | Consultar con la asesora antes de cerrar el periodo de recolección |
